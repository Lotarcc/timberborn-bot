...
Tiered ensemble: local Ollama for real-time play — a smart **planner** (`mistral-nemo:12b`, 32k on-GPU), a fast **reflex/router** (`qwen2.5:3b` or code) for routine ticks, an **embedder** (`bge-m3`) for memory retrieval, and a local **escalation planner** (`qwen2.5:14b`) for hard mid-run calls. The offline **coach** is frontier **Claude Code** (Fable 5 / Opus 4.8, high–max effort) on the Max subscription, running between runs (local 14B as unattended fallback). Cheap local models play; frontier Claude reflects. Schema-constrained decoding guarantees valid actions. Config, VRAM math, context budget, and benchmarks: `docs/compute-and-models.md`.
